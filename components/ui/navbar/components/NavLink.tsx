...
export function NavLink({ icon, label, href, badge, active, onClick }: NavLinkProps) {
  const className = `inline-flex h-11 items-center gap-2 rounded-full border px-4 text-sm font-medium transition-all duration-200 whitespace-nowrap ${
    active
      ? "border-blue-500/20 bg-linear-to-r from-blue-600 to-cyan-600 text-white shadow-lg shadow-blue-500/20"
  ...
  const iconCls = `inline-flex h-7 w-7 items-center justify-center rounded-full ${
    active ? "bg-white/15" : "bg-slate-100 text-slate-500 dark:bg-white/5 dark:text-slate-300"
  }`;
