...
export function NavMobile({ isOpen, isLoggedIn, activeNav, onNavClick, onLogin, onLogout }: NavMobileProps) {
  ...
  useEffect(() => {
    function handleOutside(e: MouseEvent) {
      if (collapsibleRef.current && !collapsibleRef.current.contains(e.target as Node)) {
        setExpanded(null);
      }
    }
  ...
  const activeCls =
    "border-blue-500/20 bg-linear-to-r from-blue-600 to-cyan-600 text-white shadow-lg shadow-blue-500/15";
    ...
    >
      <div className="max-h-[calc(100vh-4.5rem)] overflow-y-auto overscroll-contain px-3 py-3 space-y-3 bg-white/95 dark:bg-zinc-950/95 backdrop-blur-xl shadow-[0_18px_40px_-24px_rgba(15,23,42,0.35)]">
        ...
        <div className="flex items-center gap-2 px-3 h-12 rounded-2xl border border-slate-200 bg-slate-50/90 dark:border-white/10 dark:bg-white/5">
          <SearchIcon style={{ fontSize: 16 }} className="text-slate-400" />
        ...
        >
          <HomeOutlinedIcon fontSize="small" className="opacity-75" />
          Inicio
        </Link>

        <section
          ref={collapsibleRef}
          className="rounded-3xl border border-slate-200/80 bg-white/90 p-3 shadow-sm dark:border-white/10 dark:bg-white/5"
        >
          <MobileCollapsible
            id="products"
            label="Categorías"
            icon={<WorkOutlineOutlinedIcon fontSize="small" className="opacity-70" />}
            expanded={isOpen ? expanded : null}
            onToggle={toggle}
            onItemClick={() => setExpanded(null)}
            items={PRODUCT_ITEMS}
          />
        </section>
        ...
        <div className="rounded-3xl border border-slate-200/80 bg-white/90 p-3 shadow-sm dark:border-white/10 dark:bg-white/5">
          ...
          {isLoggedIn ? (
            <div className="space-y-2">
              ...
              >
                <PersonOutlineOutlinedIcon fontSize="small" className="opacity-75" />
                Mi Perfil
              </Link>
              ...
              <Link
                ...
                className="flex h-12 w-full items-center gap-3 rounded-2xl border border-slate-200 bg-slate-50/90 px-4 text-sm font-medium text-slate-700 transition-all duration-150 hover:border-blue-200 hover:bg-blue-50 hover:text-blue-700 dark:border-white/10 dark:bg-white/5 dark:text-slate-300 dark:hover:border-blue-900 dark:hover:bg-blue-950/40 dark:hover:text-blue-300"
              >
                <SettingsOutlinedIcon fontSize="small" className="opacity-75" />
                Configuración
              </Link>
              ...
              >
                <LogoutOutlinedIcon fontSize="small" className="opacity-75" />
                Salir
              </button>
            </div>
          ) : (
            <div className="space-y-2">
              <button
                onClick={onLogin}
                className="flex h-12 w-full items-center justify-center gap-2 rounded-2xl bg-linear-to-r from-blue-600 to-cyan-600 px-4 text-sm font-semibold text-white shadow-lg shadow-blue-500/20 transition-all duration-200 hover:-translate-y-0.5 active:scale-95"
              >
...
function MobileCollapsible({ id, label, icon, expanded, onToggle, onItemClick, items }: MobileCollapsibleProps) {
  ...
  return (
    <div>
      ...
      <div className={`overflow-hidden transition-all duration-200 pl-4 ${isOpen ? "max-h-72 mt-2" : "max-h-0"}`}>
