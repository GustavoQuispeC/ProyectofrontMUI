...
export function NavDesktop({ activeNav, openDropdown, onNavClick, onToggleDropdown }: NavDesktopProps) {
  ...
  return (
    <div className="hidden md:flex items-center gap-2 flex-1 rounded-full border border-white/60 bg-white/40 px-2 py-1 shadow-sm backdrop-blur-sm dark:border-white/10 dark:bg-white/5">
      ...
      <Dropdown id="products" openId={openDropdown} onToggle={onToggleDropdown} trigger={productTrigger}>
        ...
        {PRODUCT_ITEMS.map(({ icon: Icon, label, href }) => (
          <DropdownItem key={label} icon={<Icon fontSize="small" />} label={label} href={href} />
        ))}
      </Dropdown>
