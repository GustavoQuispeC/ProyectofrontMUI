...
import { AccessTime, Facebook, Instagram, Mail, StorefrontOutlined } from "@mui/icons-material";
...
const socialLinks = [
  {
    ...
    href: "https://www.facebook.com/p/Grupo-Famet-SAC-100044475944246/",
    className: "hover:bg-blue-600 hover:text-white",
  },
  {
    name: "Instagram",
    icon: <Instagram fontSize="small" />,
    href: "https://www.instagram.com/grupofametsac/",
...
export default function Footer() {
  return (
    <footer className="relative overflow-hidden border-t border-white/10 bg-slate-950 text-slate-100">
      ...
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_20%_0%,rgba(249,115,22,0.12),transparent_32%),radial-gradient(circle_at_85%_20%,rgba(14,165,233,0.12),transparent_30%)]" />
      ...
      <div className="relative mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="grid gap-10 py-2 sm:py-4 lg:grid-cols-[1.25fr_0.85fr_1fr] lg:gap-12">
          <section aria-label="Grupo Famet">
            <Link href="/" className="group inline-flex items-center">
              <Image
                ...
                height={64}
                loading="lazy"
                className="h-14 w-auto object-contain transition-transform duration-300 group-hover:scale-[1.03] brightness-110"
              />
            </Link>
              ...
              {socialLinks.map((social) => (
                <li key={social.name}>
                  <a
                    ...
                    className={`inline-flex h-10 w-10 items-center justify-center rounded-full border border-white/10 bg-white/5 text-slate-300 shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:border-transparent focus:outline-none focus:ring-2 focus:ring-orange-500 focus:ring-offset-2 focus:ring-offset-blue-950 ${social.className}`}
                  >
                    {social.icon}
                  </a>
                </li>
            ...
            <ul className="grid grid-cols-2 gap-2 sm:max-w-md lg:grid-cols-1">
              {quickLinks.map((link) => (
                <li key={link.label}>
                  <Link
                    href={link.href}
                    className="group flex items-center justify-between rounded-lg px-3 py-2 text-sm font-medium text-slate-300 transition-colors duration-200 hover:bg-white/5 hover:text-orange-300 focus:outline-none focus:ring-2 focus:ring-orange-500"
                  >
          ...
          <section aria-label="Contacto">
            ...
            <div className="mt-4 rounded-lg border border-white/10 bg-white/5 p-4 shadow-sm">
              <div className="flex items-start gap-3">
                <span className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-orange-500/15 text-orange-300">
        ...
        <div className="flex flex-col gap-4 border-t border-white/10 py-5 text-sm text-slate-400 md:flex-row md:items-center md:justify-between">
          <p className="text-center md:text-left">
            © {new Date().getFullYear()} <span className="font-semibold text-slate-200">Grupo Famet SAC</span>
          </p>
  ...
  return (
    <li>
      <a
        ...
        className="group flex items-center gap-3 rounded-lg border border-white/10 bg-white/5 p-3 shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:bg-white/[0.07] focus:outline-none focus:ring-2 focus:ring-orange-500"
      >
        <span
          className={`inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-full transition-colors duration-200 ${
            isWhatsApp
              ? "bg-emerald-500/15 text-emerald-300 group-hover:bg-emerald-600 group-hover:text-white"
              : "bg-orange-500/15 text-orange-300 group-hover:bg-orange-600 group-hover:text-white"
          }`}
        >
          {icon}
        </span>
