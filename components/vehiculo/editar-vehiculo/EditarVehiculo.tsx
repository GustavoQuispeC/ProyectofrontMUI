...
import { useEditarVehiculo, useVehiculo } from "@/features/dashboard/vehiculo/hooks/useVehiculos";
import { VehiculoForm, vehiculoSchema } from "@/features/dashboard/vehiculo/vehiculo.schema";
import { EditarVehiculoRequest } from "@/features/dashboard/vehiculo/vehiculo.type";
...
export default function EditarVehiculo({ id }: EditarVehiculoProps) {
  const router = useRouter();

  const user = getAuthUser();
  const canAccess = user ? hasPermission(user.rol, permissions.editarVehiculo) : false;

  const { vehiculo, loading: loadingVehiculo } = useVehiculo(id);
  const editarVehiculoMutation = useEditarVehiculo(id);
  const isSubmitting = editarVehiculoMutation.loading;
  ...
  const onSubmit = async (data: VehiculoForm) => {
    try {
      ...
      await toastPromise(editarVehiculoMutation.editarVehiculo(payload), {
        loading: "Actualizando vehículo...",
        success: "Vehículo actualizado correctamente.",
        error: (error) => error.message,
      });

      router.push("/dashboard/vehiculo/listar");
    } catch (error) {
      console.error(error);
    }
  };
